...
static int serial_init_dev(const int dev_index)
{
	...
	/* enable FIFOs */
//	writel(0x1, &uart->ufcon); Joshua
	writel(0, &uart->ufcon); // Joshua
	writel(0, &uart->umcon);
	/* 8N1 */
...
//					 RX_FIFO_FULL_MASK))) { Joshua
	while(!(readl(&uart->utrstat) & 0x1)) {		// Joshua
...
//	while ((readl(&uart->ufstat) & TX_FIFO_FULL_MASK)) { Joshua
	while(!(readl(&uart->utrstat) & 0x2)) {	// Joshua
